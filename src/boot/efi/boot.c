...
static size_t get_entry_index(Config *config, size_t idx_highlight, ConfigEntry *visible_group) {
        ...
        assert(idx_highlight < config->entry_count);
...
static void config_copy_entries(Config *config, ConfigEntry **entries, size_t start_index, size_t end_index, ConfigEntry *group_entry) {
        ...
        for (size_t i = start_index; i <= end_index; i++) {
                ...
                config->entries[config->entry_count - 1]->group_entry = group_entry;
        }
}
...
static void config_create_groups(Config *config, Indices *indices, size_t group_count){
        ...
        size_t old_entry_count = config->entry_count;
        ...
        if (indices[i-1].end_index != old_entry_count-1)
                config_copy_entries(config, old_entries, indices[i-1].end_index+1, old_entry_count - 1, NULL);
        free(old_entries);
}
...
static ConfigEntry *config_entry_add_loader_auto(
        ...
        if (!loader) {
                ...
                if (strcaseeq16(loader, loaded_image_path) ||
                    ...
                    is_sd_boot(root_dir, u"\\EFI\\BOOT\\GRUB.EFI") ||
